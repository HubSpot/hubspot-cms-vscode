import { ExtensionContext, commands, FileCreateEvent, WorkspaceEdit, Uri, workspace } from 'vscode';
...
const buildFileCreateListener = (extension: string, folderPath: string, doAfter: Function, cleanupCallback: Function) => {
    return async (e: FileCreateEvent) => {
        if (
            e.files?.length === 1 && new RegExp(`${folderPath}/.*`).test(e.files[0].fsPath)
        ) {
            cleanupCallback();
    ...
    return (clickContext: any) => {
        if (clickContext.scheme === 'file') {
            const createFileSubscription = workspace.onDidCreateFiles(
                buildFileCreateListener(extension, clickContext.fsPath, doAfter, () => createFileSubscription.dispose())
            );
    ...
    context.subscriptions.push(
        commands.registerCommand(
            ...
            onClickCreateFile('html', (uniqueFilePath: string) => {
                createTemplate(path.basename(uniqueFilePath), path.dirname(uniqueFilePath), 'section', { allowExisting: true });
            })
        )
    );
    context.subscriptions.push(
        commands.registerCommand(
            ...
            onClickCreateFile('html', (uniqueFilePath: string) => {
                createTemplate(path.basename(uniqueFilePath), path.dirname(uniqueFilePath), 'page-template', { allowExisting: true });
            })
        )
    );
    context.subscriptions.push(
        commands.registerCommand(
            ...
            onClickCreateFile('html', (uniqueFilePath: string) => {
                createTemplate(path.basename(uniqueFilePath), path.dirname(uniqueFilePath), 'partial', { allowExisting: true });
            })
        )
    );
    context.subscriptions.push(
        commands.registerCommand(
            ...
            onClickCreateFile('html', (uniqueFilePath: string) => {
                createTemplate(path.basename(uniqueFilePath), path.dirname(uniqueFilePath), 'global-partial', { allowExisting: true });
            })
        )
    );
    context.subscriptions.push(
    ...
    context.subscriptions.push(
        commands.registerCommand(
            ...
            onClickCreateFile('js', (filePath: string) => {
                ...
                const functionFileName = filePath.slice(
